...
abstract class BaseQuery implements IteratorAggregate
{
    ...
    private function debugger() {
        if ($this->fpdo->debug) {
            if (!is_callable($this->fpdo->debug)) {
                ...
                if (is_defined(STDERR) && is_resource(STDERR)) { // if STDERR is set, send there, otherwise just output the string
    ...
    private function clauseNotEmpty($clause) {
        if ($this->clauses[$clause] && is_array($this->statements[$clause])) {
